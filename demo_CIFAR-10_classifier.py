...
ACTIVATION_FUNC = {'Step':step, 'SimpleReturn':simplereturn, 'ReLU':ReLU, 'Approximated_ReLU':Approximated_ReLU, 'Parametric_ReLU':Parametric_ReLU, 'ELU':ELU, 'sigmoid':sigmoid, 'softsign':softsign}
USE_FUNC = ['Parametric_ReLU', 'Parametric_ReLU', 'Parametric_ReLU', 'Parametric_ReLU', 'Parametric_ReLU', 'Parametric_ReLU', 'Parametric_ReLU', 'Parametric_ReLU', 'Parametric_ReLU', 'SimpleReturn']

def main(self):
  # Import data
  file_in = '../datasets/CIFAR-10_data/'
  ...
  for i in range(NLAYERS-1):
      W = tf.Variable(np.resize(W_b_init['W' + str(i + 1) + '_init'], (NUNITS[i], NUNITS[i+1])), name='W' + str(i+1))
      b = tf.Variable(np.resize(W_b_init['b' + str(i + 1) + '_init'], (NUNITS[i+1])), name='b' + str(i+1))
  ...
  loss_summary = tf.summary.scalar("loss_"+USE_FUNC[0]+'_' + str(EPOCH) + '_' + str(LEARNING_RATE) + '_' + str(NLAYERS), loss)
  ...
  accuracy_summary = tf.summary.scalar("accuracy_" + USE_FUNC[0]+'_' + str(EPOCH) + '_' + str(LEARNING_RATE)+ '_' +str(NLAYERS), accuracy)
  ...
  writer = tf.summary.FileWriter("../tensorboard/cifar-10_logs/"+ USE_FUNC[0], sess.graph_def)
  ...
  for i in range(iter):
    ...
    if (i + 1) % (55000 / MBATCH) == 0:
        ...
        if (i + 1) / (55000 / MBATCH) % 10 == 0:
            # Save a model
            saver.save(sess,
                       '../models/' + DATASET + '/' + USE_FUNC[0] + '/'+ str(NLAYERS) + 'layers_' + str(int((i + 1) / (55000 / MBATCH))) + '.ckpt')
            #print('Saved a models.')

  print()

  # Make CSV
  with open('../models/%s/%s/%slayers_%f_%s.csv' % (DATASET, USE_FUNC[0], NLAYERS, LEARNING_RATE, USE_FUNC[0]), 'w') as f:
...
if __name__ == '__main__':
  ...
  parser.add_argument('--data_dir', type=str, default='/tmp/tensorflow/cifar-10/input_data',
