...
def main(self):
  ...
  W_b_in = '../datasets/W_glorot_init'
  with open(W_b_in, mode='rb') as f:
      W_glorot_init = pickle.load(f)
  ...
  for i in range(NLAYERS-1):
      W = tf.Variable(W_glorot_init['W' + str(i + 1) + '_glorot_init'], name='W' + str(i+1))
  ...
  writer = tf.summary.FileWriter("../tensorboard/mnist_logs/glorot_" + USE_FUNC[0], sess.graph_def)
  ...
  for i in range(iter):
    ...
    if (i + 1) % (55000 / MBATCH) == 0:
        ...
        if (i + 1) / (55000 / MBATCH) % 10 == 0:
            # Save a model
            saver.save(sess,
                       '../models/' + DATASET + '/' + USE_FUNC[0] + '/'+ str(NLAYERS) + 'layers_glorot_' + str(int((i + 1) / (55000 / MBATCH))) + '.ckpt')
            #print('Saved a models.')

  print()

  # Make CSV
  with open('../models/%s/%s/%slayers_glorot_%f_%s.csv' % (DATASET, USE_FUNC[0], NLAYERS, LEARNING_RATE, USE_FUNC[0]), 'w') as f:
