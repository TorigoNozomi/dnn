...
ACTIVATION_FUNC = {'Step':step, 'SimpleReturn':simplereturn, 'ReLU':ReLU, 'Approximated_ReLU':Approximated_ReLU, 'Parametric_ReLU':Parametric_ReLU, 'ELU':ELU, 'sigmoid':sigmoid, 'softsign':softsign, 'tanh':tanh}
#USE_FUNC = ['Parametric_ReLU', 'Parametric_ReLU', 'Parametric_ReLU', 'Parametric_ReLU', 'SimpleReturn']
USE_FUNC = ['ReLU', 'ReLU', 'ReLU', 'ReLU', 'SimpleReturn']

def main(self):
  # Import data
  file_in = '../datasets/MNIST_data/'
  mnist = input_data.read_data_sets(file_in, one_hot=True)
  W_b_in = '../datasets/W_he2_init'
  with open(W_b_in, mode='rb') as f:
      W_he2_init = pickle.load(f)
  ...
  for i in range(NLAYERS-1):
      W = tf.Variable(W_he2_init['W' + str(i + 1) + '_he2_init'], name='W' + str(i+1))
  ...
  writer = tf.summary.FileWriter("../tensorboard/mnist_logs/he_" + USE_FUNC[0], sess.graph_def)
  ...
  for i in range(iter):
    ...
    # Save a model
    saver.save(sess,
                       '../models/' + DATASET + '/' + USE_FUNC[0] + '/'+ str(NLAYERS) + 'layers_he_' + str(int(i + 1)) + '.ckpt')
    #print('Saved a models.')

  print()

  # Make CSV
  with open('../models/%s/%s/%slayers_he_%f_%s.csv' % (DATASET, USE_FUNC[0], NLAYERS, LEARNING_RATE, USE_FUNC[0]), 'w') as f:
      list_score = [list_iter, list_train_loss, list_train_accuracy, list_validation_loss,
 ...
 #Restore Model
  for i in range(iter):
      if (i + 1) / (55000 / MBATCH) % 10 == 0:
          saver.restore(sess,
                        '../models/' + DATASET + '/' + USE_FUNC[0] + '/' + str(NLAYERS) + 'layers_he_' + str(int((i + 1) / (55000 / MBATCH))) + '.ckpt')
