...
#USE_FUNC = ['Approximated_ReLU', 'Approximated_ReLU', 'Approximated_ReLU', 'Approximated_ReLU', 'SimpleReturn']
USE_FUNC = ['ELU', 'ELU', 'ELU', 'ELU', 'SimpleReturn']

def main(self):
  # Import data
  file_in = '../datasets/MNIST_data/'
  mnist = input_data.read_data_sets(file_in, one_hot=True)
  W_b_in = '../datasets/W_he_init'
  with open(W_b_in, mode='rb') as f:
      W_he_init = pickle.load(f)
  ...
  for i in range(NLAYERS-1):
      W = tf.Variable(W_he_init['W' + str(i + 1) + '_he_init'], name='W' + str(i+1))
